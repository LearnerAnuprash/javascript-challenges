...
//Example :  verify("freeCodeCamp", "Rocks", 238) should return true.
...
function verify(message, key, signature) {
  ...
  for (let m of message) {
    if (!/[a-zA-z]/.test(m)) {
      // Checking if it ain't an alphabet
      continue;
    }
    if (m === m.toLowerCase()) {
      sum += m.charCodeAt(0) - 96; //adds accordingly
    }

    if (m === m.toUpperCase()) {
      sum += m.charCodeAt(0) - 38;
    }
  }
  ...
  for (let k of key) {
    ...
    if (k === k.toLowerCase()) {
      sum += k.charCodeAt(0) - 96;
    }

    if (k === k.toUpperCase()) {
      sum += k.charCodeAt(0) - 38;
    }
  }

  return sum === signature; // returns boolean value
}
